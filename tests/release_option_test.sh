...
test_create_DebianJessie_container_using_long_option() {
  ...
  COUNT=`$WORKING_DIR/holodev run cat /etc/debian_version | grep --count "9.\|stretch"`
  assertEquals "container file '/etc/debian_version' doesn't match '9.|stretch'" 1 $COUNT
